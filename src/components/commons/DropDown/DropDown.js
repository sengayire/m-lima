...
const DropDown = ({ icon, isAuth, id }) => {
  ...
  const userOptions = [
    ...
    { text: <span>Logout</span>, redirect: '/logout' },
  ];
  return (
    <Dropdown
      direction="left"
      ...
      icon={
        (isAuth && icon) || <Icon name="chevron down" className="drop-down-chevron" size="small" />
      }
    >
      <Dropdown.Menu className="drop-down-menu">
        {(!isAuth &&
          languageOption.map(({ icon, text }) => <Dropdown.Item icon={icon} text={text} />)) ||
